...
class _ChallengeDetailScreenState extends State<ChallengeDetailScreen>
  ...
  void initState() {
    super.initState();
    challengeDeatail = _challengeRepository
        .detailChallenge(widget.climbGroundId); // 클라이밍장 ID로 상세 정보 가져오기
  ...
  Color _getMedalColor(String medal) {
    switch (medal.toUpperCase()) {
      ...
      case 'SILVER':
        return const Color(0xFFB0B0B0); // 은색
      case 'BRONZE':
        return const Color(0xFFCD7F32); // 동색
      default:
        return const Color(0xFF8B4513); // NONE일 때
    }
  }
      ...
      case 'GOLD':
        return [
          Shadow(
            ...
            blurRadius: 15,
            color: const Color(0xFFFFD700).withOpacity(0.8), // 금색 반짝임 효과
          ),
          Shadow(
            offset: const Offset(-2, -2),
            blurRadius: 10,
            color: const Color(0xFFFFE700).withOpacity(0.6), // 금색 빛나는 느낌 추가
          ),
        ];
      case 'SILVER':
        return [
          Shadow(
            offset: const Offset(2, 2),
            blurRadius: 15,
            color: const Color(0xFFC0C0C0).withOpacity(0.8), // 은색 반짝임 효과
          ),
          Shadow(
            offset: const Offset(-2, -2),
            blurRadius: 10,
            color: const Color(0xFFB0B0B0).withOpacity(0.6), // 은색 빛나는 느낌 추가
          ),
        ];
      case 'BRONZE':
        return [
          Shadow(
            offset: const Offset(2, 2),
            blurRadius: 15,
            color: const Color(0xFFCD7F32).withOpacity(0.8), // 동색 반짝임 효과
          ),
          Shadow(
            offset: const Offset(-2, -2),
            blurRadius: 10,
            color: const Color(0xFFB57B5A).withOpacity(0.6), // 동색의 밝은 효과
          ),
        ];
      case 'NONE':
        return [
          Shadow(
            offset: const Offset(2, 2),
            blurRadius: 10,
            color: const Color(0xFF8B4513).withOpacity(0.7), // 나무색 그림자 효과
          ),
          Shadow(
            offset: const Offset(-2, -2),
            blurRadius: 5,
            color: const Color(0xFF8B4513).withOpacity(0.6), // 나무색의 빛나는 느낌
          ),
        ];
      default:
                                  ...
                                  child: IconButton(
                                    ...
                                    onPressed: () {
                                      ...
                                      showModalBottomSheet(
                                        context: context,
                                        isScrollControlled:
                                            true, // 이 설정으로 모달의 크기를 자동으로 맞춤
                                        builder: (context) {
                                          return Container(
                                            width: double
                                                .infinity, // 가로가 화면 전체로 확장됨
                                            padding: const EdgeInsets.all(16.0),
                                            child: Column(
                                        ...
                                        builder: (context, child) {
                                          return Transform(
                                            transform: Matrix4.identity()
                                              ..setEntry(3, 2, 0.004)
                                              ..rotateY(
                                                  pi * 2.0 * _animation.value),
                                            alignment: Alignment.center,
                                            child: child,
                                          );
                                        },
                                        child: Image.asset(
                                            'assets/challenge/${place.medal}.png'),
                                      ),
                                    ],
                                  ),
                                ),
